/*
   bitmap image header construct
REFERENCE : http://www.cprogramdevelop.com/4596386/
REFERENCE : http://goo.gl/pkU6vY
REFERENCE : http://stackoverflow.com/questions/14279242/read-bitmap-file-into-structure
REFERENCE : received from last semester team.
 */
...
// type for bitmap image header.
// understand reference http://googl/pkU6vY
typedef struct {
	unsigned int biSize;
...
{// this code is saving a image that camera takes.
	...
	for(i=0; i<10; i++)
	{
		...
		DrawFromRGB565(image, m_buffers_preview[cam_index].virt.p, 640, \
				480, 640, 480);	// get image value from camera.

		ret = fimc_v4l2_qbuf(m_cam_fd, cam_index);	
	}

	if(!(fp = fopen("saved.img", "wb+")))	// open file, if not, print error.
	{
...
static int ir_stat = -1;


void* camapp(){
	pthread_mutex_lock(&mut[CREATE_M]);
	pthread_detach(pthread_self());

	CreateCamera(0);	// create camera. meaning, ready camera. so can take pictures.
	startPreview();		// previewing.

	pthread_mutex_unlock(&mut[CREATE_M]);

	while(1){
	
		if(ir_stat != sdata->ir_sensor){
			ir_stat = sdata->ir_sensor;

			if(user_login && (ir_stat == IR_DETECT)){// if IR detect student
				save();		// take pictures, during camera on. when, IR detect student
				fprintf(stderr, "Captured\n");
			}
		}

		if(user_login == USER_NOT_LOGIN){	// when user is not login.
			pthread_cond_wait(&con, &mut[WAIT_M]);
			pthread_mutex_unlock(&mut[WAIT_M]);
		}
	}
}